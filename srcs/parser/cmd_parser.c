...
/*   Updated: 2025/03/14 08:32:51 by yboumanz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_process_argument(t_token **current, t_cmd *cmd, int *i,
...
static t_cmd	*ft_create_command(t_token *start, t_minishell *minishell)
{
	...
	cmd->args[i] = NULL;
	ft_set_command_name(cmd, minishell);
	return (cmd);
}

static t_cmd	*ft_process_pipes(t_cmd *first_cmd, t_token *current_token,
		t_minishell *minishell)
{
	t_cmd	*current_cmd;
	...
	while (current_token)
	{
		if (current_token->type == TOKEN_PIPE)
		{
			if (!current_token->next)
				break ;
			current_cmd->next = ft_create_command(current_token->next,
					minishell);
			if (!current_cmd->next)
			{
...
t_cmd	*tokens_to_cmds(t_token *tokens, t_minishell *minishell)
{
	...
	if (!first_cmd)
		return (NULL);
	return (ft_process_pipes(first_cmd, tokens, minishell));
}
